...
class CorreoLlegaTardeController extends Controller
{
    public function armarCorreosLlegaTarde( Request $request )
    {
        ...
        foreach( $licencias as $lice ) {
            ...
            $cantidad = 0;
            $horaEnv = Carbon::parse( $lice->ult_proce.' '.$lice->henvio );
                                    ...
                                    ->select("SELECT 
                                            ...
                                            (
                                                ...
                                                WHERE 
                                                    ...
                                                    LTE.fe_hh_entrada BETWEEN DATE_SUB( CAST('{$fechaBusqueda->yesterday()->format('Y-m-d')}' AS DATE), INTERVAL 7 DAY) AND CAST('{$fechaBusqueda->yesterday()->format('Y-m-d')}' AS DATE)
                                            ) AS canti_llegadas
                                        FROM 
        ...
        return response()->json([
            ...
            'licencias_procesadas'  => $lice_proce,
        ]);
    }
}
