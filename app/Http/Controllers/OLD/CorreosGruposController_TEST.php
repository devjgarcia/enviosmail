                ...
                if( !empty($grupos) && $grupos->count() > 0 ) {
                    foreach( $grupos as $grupo ){
                        ...
                        $datos_grupo = DB::connection('mysql_din')
                                            ->select("CALL Prepasalicorreo( ?, ?, ?)", [$fecha_prc, $fecha_prc, $grupo->id_tn003]);

                        if( $datos_grupo ){
                        
                            $hora_envio = $datos_grupo[0]->HoraSalida ?? '08:00:00';
                            $asunto = 'Resumen de Grupo: ' . $grupo->nombre_sub_nivel . '. Semana del '. date('d-m-Y', strtotime($fecha_desde)) .' hasta '. date('d-m-Y', strtotime($fecha_hasta));
